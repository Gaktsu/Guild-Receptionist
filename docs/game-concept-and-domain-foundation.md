...
## 6) 피드백 반영: `EquipmentInventory` / `InjuryRecoverySystem` 재검토

요청하신 항목을 현재 게임 루프 기준으로 다시 점검한 결과, **두 객체 모두 현재 기획과 자연스럽게 연결되므로 유지**하는 쪽이 타당하다.

### `EquipmentInventory`를 유지하는 이유
- 본 게임의 핵심은 "누굴 보내는가"뿐 아니라 **어떤 준비 상태로 보내는가**까지 포함한 운영 판단이다.
- 퀘스트 난이도 재평가 후, 같은 파티라도 장비/소모품 상태에 따라 성공률과 부상률이 달라지므로 `AssignmentPlanner`와 `MissionResolver` 사이의 중간 도메인이 필요하다.
- 길드 운영(제작/구매/보급)과 원정 결과(소모/파손/획득)를 연결하는 자원 고리로도 필수다.

### `InjuryRecoverySystem`를 유지하는 이유
- "결과 확인 → 길드 관리 → 다음 날 배정" 루프에서 가장 중요한 제약은 **인력의 회복 가능성**이다.
- 부상/피로를 `Adventurer` 내부 값으로만 두면, 치료 우선순위/시설 보너스/회복 비용 같은 운영 의사결정을 표현하기 어렵다.
- 별도 시스템으로 분리하면 100일 모드에서의 단기 최적화(빠른 복귀)와 무한 모드의 장기 안정화(과로 방지)를 모두 설계하기 쉽다.

### 단순화가 필요할 때의 축소안
- 프로토타입 단계에서는 두 클래스를 제거하는 대신, 아래처럼 축소 가능:
  - `EquipmentInventory` → `GuildResourcePool` 하위 자료구조로 흡수
  - `InjuryRecoverySystem` → `AdventurerRoster.ResolveDailyRecovery()`로 임시 통합
- 다만 이 경우, 중후반 확장(장비 특성/치료 정책/시설 시너지)에서 재분리가 거의 필수라 초기에 책임 경계를 잡아두는 편이 유지보수에 유리하다.
